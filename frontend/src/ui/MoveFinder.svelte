<script lang="ts">
  ...
  /** Enter glances at the move beside whatever you were looking at; Open goes
   *  to its own page. Both are addresses, so both survive a reload. */
  function glance() {
    if (!selected) return;
    peek({ kind: 'move', id: selected.move.id });
    onclose();
  }
...
<div class="finder" role="dialog" aria-label="Find a move" aria-modal="true">
    ...
    <div class="preview">
      {#if selected}
        ...
        <div class="row pactions">
          ...
          <button class="small" onclick={glance}>Peek</button>
  ...
  <div class="row keys muted small">
    ...
    <span><span class="kbd">↵</span> peek</span>
